            ...
            Moon.objects.bulk_create(
                [
                    Moon(
                        mass=raw_moon.mass,
                        planet=orm_planet,
                        polar_radius=raw_moon.polar_radius,
                    )
                    for raw_moon in raw_planet.moons
                ]
            )

        Asteroid.objects.bulk_create(
            [
                Asteroid(mass=asteroid_raw.mass, name=asteroid_raw.name)
                for asteroid_raw in AsteroidRaw.from_raw(data)
            ]
        )
